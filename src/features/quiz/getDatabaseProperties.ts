...
export const judgeDBorQuiz = async (db_id: string) => {
  ...
  let i = 0;
  while (isAllHasData == true && i < resNum) {
...
export const getQuizCorrectAnswer = async (db_id: string, num: number) => {
  ...
  let i = 1;
  while (isHasMoreCorrectAnswer == true) {
    properties["correct_" + i]
      ? properties["correct_" + i].rich_text[0]
        ? (rightArray[i - 1] =
            properties["correct_" + i].rich_text[0].plain_text)
